...
const UI = {
    ...
    showLoading(text) {
        ...
        if (resultCard) {
            ...
            const cardInner = resultCard.querySelector('.result-card-inner') as HTMLElement;
            ...
            if (this.els.resultSection) this.els.resultSection.classList.remove('hidden');
        }
    },
    ...
    showResult(item, type) {
        if (!this.els.resultSection || !this.els.resultCard) return;
        ...
        const cardInner = this.els.resultCard.querySelector('.result-card-inner') as HTMLElement;
        if (cardInner) {
        ...
        if (type === 'movie') {
            ...
            if (ratingNumEl) ratingNumEl.textContent = Number(ratingVal).toFixed(1);
            ...
            if (tvRatingNumEl) tvRatingNumEl.textContent = Number(tvRatingVal).toFixed(1);
            this.els.resultDesc.textContent = item.overview || item.description || (typeof I18N !== 'undefined' ? I18N.t('noDesc') : '');
            ...
            if (bookRatingNumEl) bookRatingNumEl.textContent = rating;
        ...
        const aiBtn = document.getElementById('ai-analyze-btn') as HTMLAnchorElement | null;
        if (aiBtn) {
    ...
    updateSaveBtn(isSaved) {
        if (this.els.saveBtn) this.els.saveBtn.classList.toggle('saved', isSaved);
    },
    ...
    renderSavedList(items, type, isWatched = false) {
        ...
        if (type === 'tv') fallback = CONFIG.FALLBACK_TV_POSTER;
        else if (type === 'books') fallback = CONFIG.FALLBACK_BOOK_POSTER;
        ...
        if (!isWatched) this._initDragAndDrop();
    },
    ...
    _initDragAndDrop() {
        ...
        list.querySelectorAll('.saved-item[draggable]').forEach((item: HTMLElement) => {
            ...
            let touchStartY = 0;
            let touchSrcIndex: number | null = null;
            ...
            if (handle) {
                ...
                handle.addEventListener('touchmove', (e: TouchEvent) => {
                    ...
                    list.querySelectorAll('.saved-item').forEach(el => el.classList.remove('drag-over'));
                    const overItem = target?.closest('.saved-item') as HTMLElement | null;
...
(UI as any).renderSavedList = function(items, type, isWatched = false) {
    ...
    let fallback = CONFIG.FALLBACK_POSTER;
    if (type === 'tv') fallback = CONFIG.FALLBACK_TV_POSTER;
    else if (type === 'books' || type === 'book') fallback = CONFIG.FALLBACK_BOOK_POSTER;
    const savedType = type === 'tv' ? 'tv' : (type === 'books' || type === 'book') ? 'book' : 'movie';
    const detailPrefix = savedType === 'tv' ? 'desctv' : savedType === 'book' ? 'descbook' : 'descmovie';
