...
const Game = {
    ...
    initLobbyUI() {
        ...
        if (!menuScreen || document.getElementById('game-lobby-section')) return;
    ...
    async createLobby() {
        try {
            ...
            if (codeDisplay) codeDisplay.textContent = d.lobby_code;
    ...
    async joinLobby() {
        const input = document.getElementById('game-join-code-input') as HTMLInputElement | null;
    ...
    async loadMyRating() {
        try {
            ...
            if (!res.ok) return;
            const d = await res.json();
            if (this.myRating) this.myRating.textContent = d.rating;
            if (this.myRecord) this.myRecord.textContent = `${d.wins}W / ${d.losses}L / ${d.draws}D`;
            ...
            if (totalEl) totalEl.textContent = `${d.wins + d.losses + d.draws} ігор`;

            const findBtn = document.getElementById('game-find-btn') as HTMLButtonElement | null;
            if (d.game_banned) {
                if (findBtn) {
    ...
    async pollMatch() {
        ...
        try {
            ...
            if (d.status === 'waiting') return; // still waiting
    ...
    async onMatchReady() {
        ...
        try {
            ...
            const poster = document.getElementById('game-movie-poster') as HTMLImageElement | null;
            const title = document.getElementById('game-movie-title');
            if (poster) poster.src = d.movie_poster || '';
            if (title) title.textContent = d.movie_title || '???';

            // Reset inputs
            const ratingInput = document.getElementById('game-input-rating') as HTMLInputElement | null;
            const yearInput = document.getElementById('game-input-year') as HTMLInputElement | null;
    ...
    showWaiting() {
        ...
        if (waitLabel) {
            ...
            const tFn = typeof I18N !== 'undefined' ? (k: string) => I18N.t(k) : (k: string) => k;
    ...
    async submitGuess() {
        const ratingVal = parseFloat((document.getElementById('game-input-rating') as HTMLInputElement)?.value);
        const yearVal = parseInt((document.getElementById('game-input-year') as HTMLInputElement)?.value);
    ...
    showResult(d) {
        ...
        (document.getElementById('game-result-poster') as HTMLImageElement).src = d.movie_poster || '';
