...
const Analytics = {
    ...
    trackEvent(eventType: string, data: any = null) {
        if (!this.enabled) return;
        const payload = {
        ...
        navigator.sendBeacon(
            `${CONFIG.API_URL}/api/analytics/event`,
            new Blob([JSON.stringify(payload)], { type: 'application/json' }),
        );
    },

    trackFunnel(step: string) {
        ...
        navigator.sendBeacon(
            `${CONFIG.API_URL}/api/analytics/funnel`,
            new Blob([JSON.stringify(payload)], { type: 'application/json' }),
        );
    },
            ...
            if (!flushTimer) {
                flushTimer = setTimeout(() => {
                    clickBuffer.forEach(c => {
                        navigator.sendBeacon(
                            `${CONFIG.API_URL}/api/analytics/click`,
    ...
    trackButtonClick(btnId: string, label: string = '') {
        this.trackEvent('button_click', { button: btnId, label: label });
    },

    trackPick(type: string, title: string = '') {
        this.trackEvent('pick', { type: type, title: title });
    },

    trackSave(type: string, title: string = '') {
