...
const Enhancements = {
    ...
    haptic(pattern: number | number[]) {
        if (Enhancements.prefersReducedMotion()) return;
        try {
    ...
    _reducedMotion: null as boolean | null,
    ...
    initReceipts() {
        ...
        document.addEventListener('visibilitychange', () => {
            if (!document.hidden) Enhancements._pollReceiptStatus();
        });
    },
    ...
    _handleReceiptRoute() {
        ...
        const receipt = Enhancements._receipts.find((r: any) => r.id === receiptId);
        if (receipt) {
            setTimeout(() => Enhancements.showReceiptModal(receipt), 300);
        } else {
            setTimeout(() => {
                if (typeof UI !== 'undefined') UI.showToast('Квитанцію не знайдено');
            }, 300);
        }
        if (pathMatch) {
    ...
    _listenReceiptMessages() {
        window.addEventListener('message', (ev: MessageEvent) => {
            ...
            if (data.type !== 'picksy-receipt' || !data.receipt || !data.receipt.id) return;
            const r = data.receipt;
            if (Enhancements._receipts.some((x: any) => x.id === r.id)) return;
            Enhancements._receipts.push(r);
            localStorage.setItem('picksy_receipts', JSON.stringify(Enhancements._receipts));
        });
    },

    createReceipt(plan: string, days: number, price: string, forcedId?: string) {
        // If a receipt with this id already exists (e.g. created via postMessage),
        // do not duplicate it — just return the existing one.
        if (forcedId) {
            const existing = Enhancements._receipts.find((r: any) => r.id === forcedId);
            if (existing) {
    ...
    confirmReceipt(receiptId: string) {
        ...
        if (receipt && !receipt.confirmed) {
            ...
            try { Enhancements.hapticSuccess(); } catch {}
            if (typeof UI !== 'undefined') {
                try { UI.showToast('Чек ' + receipt.id + ' підтверджено!'); } catch {}
            }
        }
        return receipt;
    },

    rejectReceipt(receiptId: string) {
        const receipt = Enhancements._receipts.find(r => r.id === receiptId);
        if (receipt && !receipt.rejected && !receipt.confirmed) {
            ...
            localStorage.setItem('picksy_receipts', JSON.stringify(Enhancements._receipts));
            Enhancements._refreshOpenReceiptModal(receipt);
            try { Enhancements.hapticError(); } catch {}
            if (typeof UI !== 'undefined') {
                try { UI.showToast('Чек ' + receipt.id + ' відхилено'); } catch {}
            }
        }
        return receipt;
    },

    _refreshOpenReceiptModal(receipt: any) {
        const open = document.getElementById('receipt-modal') as HTMLElement | null;
    ...
    _startReceiptPolling() {
        if (Enhancements._receiptPollTimer) return;
    ...
    async _pollReceiptStatus() {
        const pending = Enhancements._receipts.filter((r: any) => !r.confirmed && !r.rejected);
        if (pending.length === 0) return;
        const ids = pending.map((r: any) => r.id).slice(0, 50);
        try {
            ...
            for (const r of pending) {
                ...
                if (info.confirmed && !r.confirmed) {
                    ...
                    try { Enhancements.hapticSuccess(); } catch {}
                    if (typeof UI !== 'undefined') {
                        try { UI.showToast('Чек ' + r.id + ' підтверджено!'); } catch {}
                    }
                    ...
                    Enhancements._refreshOpenReceiptModal(r);
                    if (typeof UI !== 'undefined') {
                        try { UI.showToast('Чек ' + r.id + ' відхилено'); } catch {}
                    }
                }
            }
            if (changed) {
    ...
    showReceiptModal(receipt: any) {
        ...
        (modal as HTMLElement).dataset.receiptId = receipt.id;
        modal.innerHTML = `
        ...
        document.getElementById('receipt-copy-link')?.addEventListener('click', () => {
            ...
            navigator.clipboard.writeText(link).then(() => {
                if (typeof UI !== 'undefined') UI.showToast('Посилання скопійовано!');
    ...
    showReceiptsList() {
        ...
        modal.querySelectorAll('.receipt-list-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                if (receipt) Enhancements.showReceiptModal(receipt);
            });
        });
    },

    // ─── Combined Moods (multi-select 2-3) ───
    _selectedMoods: [] as number[],
    MAX_MOODS: 3,
    ...
    initCombinedMoods() {
        ...
        moodGrid.addEventListener('click', (e) => {
            const btn = (e.target as HTMLElement).closest('.mood-btn') as HTMLElement;
            if (!btn) return;
            e.stopPropagation();
            e.preventDefault();
            ...
            if (pos >= 0) {
                ...
                if (Enhancements._selectedMoods.length >= Enhancements.MAX_MOODS) {
                    ...
                    moodGrid.querySelectorAll('.mood-btn').forEach(b => {
                        if (parseInt((b as HTMLElement).dataset.index) === oldest) b.classList.remove('active');
                    });
                }
    ...
    _applyCombinedMoods() {
        ...
        if (tab === 'books') {
            ...
            moods.forEach(i => {
                if (moodList[i]?.genres) allGenres.push(...moodList[i].genres);
            });
    ...
    estimateReadingTime(pageCount: number): string {
    ...
    showBookReader(item: any) {
        ...
        Enhancements._translateBookText(modal.querySelector('.book-reader-text') as HTMLElement, description);
    },
    ...
    async _translateBookText(el: HTMLElement | null, original: string) {
        if (!el || !original) return;
        if (typeof I18N === 'undefined' || typeof I18N.translateText !== 'function') return;
        const lang = I18N.current || 'uk';
        // Only translate when target lang differs from source detection.
        if (lang === 'uk' && /[\u0400-\u04FF]/.test(original)) return;
        if (lang === 'en' && !/[\u0400-\u04FF]/.test(original)) return;
        try {
    ...
    initParallax() {
        ...
        if (hero) (hero as HTMLElement).style.willChange = 'transform, opacity';
        ...
        window.addEventListener('scroll', () => {
            if (!ticking) {
                requestAnimationFrame(() => {
                    ...
                    if (Math.abs(scrollY - lastScrollY) < 2) { ticking = false; return; }
                    lastScrollY = scrollY;

                    if (scrollY < window.innerHeight * 1.5) {
                        blobs.forEach((blob, i) => {
                            const speed = 0.15 + i * 0.05;
                            (blob as HTMLElement).style.transform = `translate3d(0,${scrollY * speed}px,0)`;
                        });
    ...
    _injectMicroStyles() {
        if (document.getElementById('picksy-micro-styles')) return;
    ...
    initSkeletonStyles() {
        if (document.getElementById('picksy-skeleton-styles')) return;
        const style = document.createElement('style');
    ...
    maybeShowAfterFirstPick() {
        try {
            if (localStorage.getItem('picksy_onboarding_done')) return;
            if (Enhancements._onboardingShown) return;
    ...
    _showOnboarding() {
        ...
        const goToStep = (step: number) => {
            ...
            setTimeout(() => {
                ...
                if (step === steps.length - 1) nextBtn.textContent = 'Почати! 🎬';
    ...
    _closeOnboarding(overlay: HTMLElement) {
    ...
    hookPayment() {
        ...
        if (!origSelectPlan) return;

        Subscription.selectPlan = function(tier: string, btn: any) {
            if (tier === 'free') return;
    ...
    _animateBusy: false,
    _isCardVisible(): boolean {
        const section = document.getElementById('result-section');
        const card = document.getElementById('result-card');
        if (!section || !card) return false;
        if (section.classList.contains('hidden')) return false;
        const inner = card.querySelector('.result-card-inner') as HTMLElement | null;
        if (!inner) return true;
        return inner.style.display !== 'none' && inner.offsetParent !== null;
    },
    _flyOutCard(): Promise<void> {
        return new Promise((resolve) => {
            const card = document.getElementById('result-card');
            if (!card) { resolve(); return; }
            card.classList.remove(
                'swipe-enter', 'swipe-enter-from-left', 'swipe-enter-from-right',
                'pick-fly-out-left', 'pick-fly-out-right',
            );
    ...
    hookPickAnimation() {
        ...
        UI.showResult = function(item: any, type: string) {
        ...
        UI.showResult = function(item: any, type: string) {
            ...
            if (type === 'book') {
                if (meta && item) {
                    ...
                    if (actionsDiv) actionsDiv.appendChild(readerBtn);
                }

                // Translate the book description in the result card to UA when needed.
                const descEl = document.getElementById('result-desc') as HTMLElement | null;
    ...
    hookButtonHaptics() {
        document.addEventListener('click', (e) => {
            ...
            if (target.closest('.save-btn')) Enhancements.hapticMedium();
            else if (target.closest('.share-btn')) Enhancements.hapticLight();
            else if (target.closest('.more-btn')) Enhancements.hapticTick();
            else if (target.closest('.lucky-btn')) Enhancements.hapticMedium();
            else if (target.closest('.duo-btn')) Enhancements.hapticMedium();
            else if (target.closest('.modal-close')) Enhancements.hapticLight();
            else if (target.closest('.auth-submit')) Enhancements.hapticMedium();
        });
    },
