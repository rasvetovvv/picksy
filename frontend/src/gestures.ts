...
const Gestures = {
    ...
    _injectSwipeStyles() {
        if (document.getElementById('picksy-swipe-styles')) return;
    ...
    initCardSwipe() {
        ...
        const onStart = (e: TouchEvent | MouseEvent) => {
            if (self._cardBusy) return;
            if ('button' in e && e.button !== 0) return;
            const target = e.target as HTMLElement;
            if (target.closest('button') || target.closest('a') || target.closest('.result-actions')) return;
        ...
        const onMove = (e: TouchEvent | MouseEvent) => {
            if (self._cardBusy || !isDragging) return;
            const pos = 'touches' in e ? { x: e.touches[0].clientX, y: e.touches[0].clientY } : { x: (e as MouseEvent).clientX, y: (e as MouseEvent).clientY };
            deltaX = pos.x - startX;
    ...
    _ensureSwipeOverlays(card: HTMLElement) {
        if (card.querySelector('.swipe-overlay-like')) return;
    ...
    _updateSwipeOverlays(card: HTMLElement, dx: number) {
        const absDelta = Math.abs(dx);
        const progress = Math.min(1, absDelta / 150);

        const likeOverlay = card.querySelector('.swipe-overlay-like') as HTMLElement;
        const skipOverlay = card.querySelector('.swipe-overlay-skip') as HTMLElement;
        const likeStamp = card.querySelector('.swipe-stamp-like') as HTMLElement;
        const skipStamp = card.querySelector('.swipe-stamp-skip') as HTMLElement;
        const glowLike = card.querySelector('.swipe-glow-like') as HTMLElement;
        const glowSkip = card.querySelector('.swipe-glow-skip') as HTMLElement;

        if (dx > 30) {
            if (likeOverlay) { likeOverlay.classList.add('active'); likeOverlay.style.opacity = String(progress); }
            if (skipOverlay) { skipOverlay.classList.remove('active'); skipOverlay.style.opacity = '0'; }
            if (likeStamp) likeStamp.classList.toggle('visible', progress > 0.3);
            if (skipStamp) skipStamp.classList.remove('visible');
            if (glowLike) glowLike.style.opacity = String(progress * 0.6);
            if (glowSkip) glowSkip.style.opacity = '0';
        } else if (dx < -30) {
            if (skipOverlay) { skipOverlay.classList.add('active'); skipOverlay.style.opacity = String(progress); }
            if (likeOverlay) { likeOverlay.classList.remove('active'); likeOverlay.style.opacity = '0'; }
            if (skipStamp) skipStamp.classList.toggle('visible', progress > 0.3);
            if (likeStamp) likeStamp.classList.remove('visible');
            if (glowSkip) glowSkip.style.opacity = String(progress * 0.6);
    ...
    _hideSwipeOverlays(card: HTMLElement) {
        const overlays = card.querySelectorAll('.swipe-overlay');
        overlays.forEach((o: Element) => {
            (o as HTMLElement).classList.remove('active');
            (o as HTMLElement).style.opacity = '0';
        });
        const stamps = card.querySelectorAll('.swipe-stamp');
        stamps.forEach((s: Element) => (s as HTMLElement).classList.remove('visible'));
        const glows = card.querySelectorAll('.swipe-glow');
        glows.forEach((g: Element) => (g as HTMLElement).style.opacity = '0');
    },

    _showShadowStack(card: HTMLElement, show: boolean) {
        const shadow = card.parentElement?.querySelector('.card-shadow-stack') as HTMLElement;
        if (shadow) shadow.classList.toggle('visible', show);
    },

    _showSwipeInstruction(card: HTMLElement) {
        if (card.querySelector('.swipe-instruction')) return;
    ...
    _emitSwipeParticles(card: HTMLElement, direction: string) {
    ...
    initTabSwipe() {
        ...
        header.addEventListener('touchend', (e: TouchEvent) => {
            ...
            if (Math.abs(endDx) < 50) return;
            let newIdx = currentIdx;
            if (endDx < -50 && currentIdx < tabs.length - 1) newIdx = currentIdx + 1;
            else if (endDx > 50 && currentIdx > 0) newIdx = currentIdx - 1;
            if (newIdx !== currentIdx && typeof App !== 'undefined') {
                App.switchTab(tabs[newIdx]);
                document.querySelectorAll('.tab-btn').forEach(b => {
                    (b as HTMLElement).classList.toggle('active', (b as HTMLElement).dataset.tab === tabs[newIdx]);
                });
            }
        });
    },
    ...
    initPullToRefresh() {
        ...
        const _isScrollableTarget = (el: HTMLElement): boolean => {
        ...
        document.addEventListener('touchmove', (e: TouchEvent) => {
            ...
            if (pulling && dy > 0) {
                ...
                if (pullDistance >= this.PULL_DISTANCE) {
                    ...
                    if (text) text.textContent = I18N.t('releaseToRefresh') || '\u0412\u0456\u0434\u043f\u0443\u0441\u0442\u0456\u0442\u044c \u0434\u043b\u044f \u043e\u043d\u043e\u0432\u043b\u0435\u043d\u043d\u044f';
                    const icon = ind.querySelector('.pull-refresh-icon');
                    if (icon) icon.textContent = '\u2191';
                } else {
                    ind.classList.remove('pull-ready');
                    const text = ind.querySelector('.pull-refresh-text');
                    if (text) text.textContent = I18N.t('pullToRefresh') || '\u041f\u043e\u0442\u044f\u0433\u043d\u0456\u0442\u044c \u0434\u043b\u044f \u043e\u043d\u043e\u0432\u043b\u0435\u043d\u043d\u044f';
                    const icon = ind.querySelector('.pull-refresh-icon');
                    if (icon) icon.textContent = '\u2193';
                }
        ...
        document.addEventListener('touchend', () => {
            ...
            if (pullDistance >= this.PULL_DISTANCE) {
                ...
                if (text) text.textContent = I18N.t('refreshing') || '\u041e\u043d\u043e\u0432\u043b\u0435\u043d\u043d\u044f...';
    ...
    initLongPress() {
        ...
        document.addEventListener('touchstart', (e: TouchEvent) => {
            ...
            if (menu && !(e.target as HTMLElement).closest('.context-menu')) {
                menu.remove();
            }
        });
    },
    ...
    _handleContextAction(action: string) {
        switch (action) {
            case 'save':
                App.toggleSave();
                break;
