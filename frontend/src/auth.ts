...
const Auth = {
    ...
    bindEvents() {
        ...
        if (profileOverlay) profileOverlay.addEventListener('click', () => profileModal.classList.add('hidden'));
        if (profileClose) profileClose.addEventListener('click', () => profileModal.classList.add('hidden'));
        if (profileLogout) profileLogout.addEventListener('click', () => this.logout());
    },
    ...
    updateModalMode() {
        ...
        const privacyCheck = document.getElementById('auth-privacy-check') as HTMLInputElement | null;
        if (privacyRow) privacyRow.classList.toggle('hidden', isLogin);
        if (privacyCheck && isLogin) privacyCheck.checked = false;
        this._setMascot('idle');
    },
    ...
    _mascotMood(state: 'idle' | 'typing' | 'success' | 'error') {
        switch (state) {
    ...
    _ensureMascot() {
        ...
        const P = (window as any).Picksy;
        if (!faceEl || !P || !P.render) return null;
    ...
    _setMascot(state: 'idle' | 'typing' | 'success' | 'error', payload?: { hintKey?: string; reason?: string }) {
        const box = document.getElementById('auth-mascot');
        const faceEl = document.getElementById('auth-mascot-face');
        const textEl = document.getElementById('auth-mascot-text');
        if (!box || !faceEl || !textEl) return;
    ...
    _mapAuthError(detail: string | undefined, httpStatus?: number) {
        const d = (detail || '').toString().toLowerCase();
        if (httpStatus === 429 || d.includes('too many requests')) return { hintKey: 'authMascotHintRateLimit', reasonI18n: 'errorTooManyRequests' };
        if (d.includes('account suspended')) return { hintKey: 'authMascotHintBanned', reasonI18n: 'errorAccountSuspended' };
        if (d.includes('registrations are currently disabled') || d.includes('registrations are temporarily')) return { hintKey: 'authMascotHintRegDisabled', reasonI18n: 'errorRegistrationsDisabled' };
        if (d.includes('email and password required')) return { hintKey: 'authMascotHintEmptyFields', reasonI18n: 'errorEmailAndPasswordRequired' };
        if (d.includes('invalid email format')) return { hintKey: 'authMascotHintInvalidEmail', reasonI18n: 'errorInvalidEmailFormat' };
        if (d.includes('password must be at least')) return { hintKey: 'authMascotHintPasswordShort', reasonI18n: 'errorPasswordTooShort' };
        if (d.includes('email already registered') || d.includes('already registered')) return { hintKey: 'authMascotHintEmailExists', reasonI18n: 'errorEmailAlreadyRegistered' };
        if (d.includes('invalid email or password')) return { hintKey: 'authMascotHintWrongCreds', reasonI18n: 'errorInvalidEmailOrPassword' };
        if (d.includes('privacy') && d.includes('accept')) return { hintKey: 'authMascotHintPrivacy', reasonI18n: 'privacyRequired' };
        if (d.includes('database unavailable')) return { hintKey: 'authMascotHintNetwork', reasonI18n: 'errorDatabaseUnavailable' };
        if (d.includes('google auth') || d.includes('google token') || d.includes('verify google')) return { hintKey: 'authMascotHintGoogleFail', reasonI18n: 'errorGoogleVerifyFailed' };
        if (d.includes('token audience mismatch')) return { hintKey: 'authMascotHintGoogleFail', reasonI18n: 'errorTokenAudienceMismatch' };
    ...
    async submit() {
        ...
        if (isRegister) body.accepted_privacy = true;
        ...
        try {
            ...
            if (typeof App !== 'undefined') App.syncFavorites();
            if (typeof Collections !== 'undefined') Collections.onAuthChange();
            if (typeof Subscription !== 'undefined') Subscription.init();
            if (typeof Analytics !== 'undefined') {
                if (this.mode === 'register') Analytics.onRegister();
            }
    ...
    async loadProfile() {
        try {
            ...
            if (res.ok) {
                ...
                if (typeof App !== 'undefined') await App.syncFavorites();
                if (typeof Subscription !== 'undefined') Subscription.init();
                if (typeof Collections !== 'undefined') Collections.onAuthChange();
            } else {
                this.logout();
    ...
    updateAdminUI() {
        ...
        if (fabAdmin) fabAdmin.classList.toggle('hidden', !this.isStaff());
        const staffFab = document.getElementById('staff-fab');
        if (staffFab) staffFab.classList.toggle('hidden', !this.isStaff());
    },
    ...
    updateUI() {
        ...
        if (typeof App !== 'undefined') {
            if (this.user) {
                ...
                if (!document.getElementById('guest-picks-badge')) App.initGuestPicksBadge();
                else if (typeof App._updateGuestBadge === 'function') App._updateGuestBadge();
            }
        }
    },
    ...
    async showUserMenu() {
        ...
        if (typeof Subscription !== 'undefined' && Subscription.refresh) {
            try { await Subscription.refresh(); } catch {}
        }
    ...
    _wireProfileTabs() {
        if (this._profileTabsWired) return;
    ...
    _switchProfileTab(tab) {
        ...
        if (receipts) receipts.classList.toggle('hidden', tab !== 'receipts');
    ...
    _wireHistoryControls() {
        ...
        if (more) more.addEventListener('click', () => this._loadHistoryPage(false));
        const clear = document.getElementById('profile-history-clear');
        if (clear) clear.addEventListener('click', async () => {
            if (!confirm(I18N.t('historyConfirmClear'))) return;
        ...
        items.forEach(it => {
            ...
            if (itemId) {
                el.addEventListener('click', (ev: any) => {
                    if (ev.target.closest && ev.target.closest('.phi-link')) return;
    ...
    async loadLevel() {
        ...
        if (!stats || !stats.level) return;
        const lv = stats.level;
        const set = (id, val) => { const el = document.getElementById(id); if (el !== null && el !== undefined) el.textContent = val; };
        ...
        if (tn) tn.textContent = next ? '· ' + I18N.t('toNext').replace('{n}', toNext) : '· ' + I18N.t('maxLevel');
        const fill = document.getElementById('profile-progress-fill');
        if (fill) fill.style.width = (Math.max(0, Math.min(1, lv.progress)) * 100).toFixed(1) + '%';
        ...
        if (stats.streak) {
            ...
            if (typeof App !== 'undefined' && App.getStreakClass) {
                ...
                if (fireEl) {
                    ...
                    if (color) fireEl.style.filter = `drop-shadow(0 0 6px ${color})`;
                    else fireEl.style.filter = '';
                }
            }
        }
    },
    ...
    async loadMiniProfile() {
        ...
        if (this.user && this.user.email) set('profile-email', this.user.email);
        ...
        try {
            ...
            if (displayInput) displayInput.value = data.profile?.display_name || data.user?.display_name || '';
            if (bioInput) bioInput.value = data.profile?.bio || data.user?.bio || '';
            if (avatarInput) avatarInput.value = data.profile?.avatar_emoji || data.user?.avatar_emoji || '';
            const avatar = document.querySelector('.profile-avatar');
            if (avatar && (data.profile?.avatar_emoji || data.user?.avatar_emoji)) avatar.textContent = data.profile?.avatar_emoji || data.user?.avatar_emoji;
            // Hydrate v53 profile fields (banner, accent, fav_film, social) from server.
            const setVal = (id, v) => { const el = document.getElementById(id) as HTMLInputElement | null; if (el && v != null) el.value = String(v); };
            ...
            try {
                ...
                const favPosterEl = document.getElementById('profile-fav-poster') as HTMLImageElement | null;
                ...
                if (favSelected) {
                    if (favId && favTitle) {
                        ...
                        if (favTitleEl) favTitleEl.textContent = favTitle;
            ...
            try {
                ...
                const avPreview = document.getElementById('profile-avatar-preview') as HTMLImageElement | null;
                if (avPreview) {
                    if (p.avatar_url) {
                ...
                if (typeof Extras !== 'undefined') {
                    if (typeof (Extras as any)._applyAvatarToProfile === 'function') {
                        (Extras as any)._applyAvatarToProfile(p.avatar_url || '');
                    }
                    if (typeof (Extras as any)._applyBannerToProfile === 'function') {
                        (Extras as any)._applyBannerToProfile(p.banner_url || '');
                    }
                }
            } catch {}
            ...
            const receiptsTabBtn = document.querySelector<HTMLElement>('.profile-tab-btn[data-pmtab="receipts"]');
            const receiptsPanel = document.getElementById('profile-panel-receipts');
            if (receiptsTabBtn) receiptsTabBtn.classList.toggle('hidden', !freemiumOn);
            if (receiptsPanel && !freemiumOn) receiptsPanel.classList.add('hidden');
            ...
            if (unameInput && uname) {
                ...
                if (typeof App !== 'undefined') App.showPublicLink(uname);
            }
        } catch (err) {
    ...
    renderProfileBadges(data) {
        ...
        const row = document.querySelector('.profile-head .profile-badges-row') as HTMLElement | null;
        if (!row) return;
        ...
        if (legacy && legacy !== row) legacy.remove();
        const tierBadge = document.getElementById('profile-tier-badge');
        const insertBadge = (badge: HTMLElement) => {
    ...
    bindProfileTools() {
        if (this._profileToolsBound) return;
    ...
    _googleClientId: null as string | null,
    ...
    _initGoogleSdk() {
        const tryInit = () => {
            ...
            google.accounts.id.initialize({
                ...
                callback: (response: any) => this._handleGoogleResponse(response),
                auto_select: false,
    ...
    _renderGoogleButton() {
        if (this._googleButtonRendered) return;
        // @ts-ignore
        if (typeof google === 'undefined' || !google.accounts || !this._googleClientId) return;
    ...
    async googleSignIn() {
        ...
        if (!this._googleClientId) {
            try {
                ...
                const data = await res.json();
                if (data.google_client_id) this.initGoogle(data.google_client_id);
            } catch (e) {}
        }
        if (!this._googleClientId) {
        ...
        if (!this._googleInitialized) this._initGoogleSdk();
        this._renderGoogleButton();
    },

    async _handleGoogleResponse(response: any) {
        ...
        try {
            ...
            if (typeof Collections !== 'undefined') Collections.onAuthChange();
            if (typeof Subscription !== 'undefined') Subscription.init();
            if (typeof Analytics !== 'undefined') Analytics.onRegister();
            setTimeout(() => this.hideModal(), 1200);
        } catch (err) {
            this._setMascot('error', { hintKey: 'authMascotHintNetwork', reason: I18N.t('errorNetwork') || '' });
        }
