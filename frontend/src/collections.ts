...
const Collections = {
    ...
    saveCollection(col) {
        if (this.savedCollections.some(c => c.id === col.id)) return;
    ...
    _updateSavedCount() {
        ...
        if (countEl) countEl.textContent = String(this.savedCollections.length);
    },
    save() {
    ...
    _isOwner(col) {
        ...
        if (this.data.some(c => c.id === col.id)) return true;
        // For server-side collections, check ownerId/owner_id
        if (typeof Auth !== 'undefined' && Auth.user) {
            const uid = String(Auth.user.id);
            if (col.ownerId) return String(col.ownerId) === uid;
            if (col.owner_id) return String(col.owner_id) === uid;
            if (col.owner && col.owner.id) return String(col.owner.id) === uid;
        }
        return false;
    },
    _isInvited(col) {
    ...
    async _mirrorAddToPublished(col, item) {
        if (!col || !col._publishedId) return;
        try {
    ...
    async _loadPublishedFromServer() {
        ...
        try {
            ...
            const byName = {};
            pubs.forEach(p => {
                if (p.local_id) byLocal[p.local_id] = p;
    ...
    _bindEvents() {
        ...
        if (toggle) {
            toggle.addEventListener('click', (e) => {
                ...
                if (target && target.closest && target.closest('#collections-create-btn')) return;
                const body = document.getElementById('collections-body');
                if (!body) return;
        ...
        if (tabMine) tabMine.addEventListener('click', () => this._switchTab('mine'));
        if (tabShared) tabShared.addEventListener('click', () => this._switchTab('shared'));
        if (tabSaved) tabSaved.addEventListener('click', () => this._switchTab('saved'));
        // Modal close buttons
    ...
    _switchTab(tab) {
        ...
        if (mineList) mineList.classList.add('hidden');
        ...
        if (tab === 'mine') {
            ...
            if (mineList) mineList.classList.remove('hidden');
        } else if (tab === 'shared') {
            document.getElementById('col-tab-shared')?.classList.add('active');
            if (sharedList) sharedList.classList.remove('hidden');
    ...
    renderCollectionsList() {
        ...
        list.querySelectorAll('.col-open-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                ...
                if (colId) this._navigateToCollection(colId);
            });
        });
    ...
    renderSavedList() {
        ...
        list.querySelectorAll('.col-open-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const colId = btn.dataset.colId;
                if (colId) this._openSavedCollectionDetail(colId);
            });
        });
        // Bind card clicks
        list.querySelectorAll('.collection-card').forEach((card) => {
            card.addEventListener('click', (e) => {
                if (e.target.closest('.col-action-btn')) return;
        ...
        list.querySelectorAll('.col-unsave-btn').forEach((btn) => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const colId = btn.dataset.colId;
                if (colId) this.unsaveCollection(colId);
            });
        });
    },
    _openSavedCollectionDetail(colId) {
        ...
        if (title) title.textContent = `${col.emoji} ${col.name}`;
        const countEl = modal.querySelector('.col-detail-count');
        if (countEl) countEl.textContent = `${(col.items || []).length} ${this._t('collectionsItems')}`;
    ...
    _renderSavedDetailItems(col) {
        ...
        container.querySelectorAll('.col-detail-item').forEach((row) => {
            row.addEventListener('click', (e) => {
                ...
                if (typeof App === 'undefined' || typeof App.goToDetailPage !== 'function') return;
    ...
    async _loadSharedFromServer() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (this._activeTab === 'shared') this.renderSharedList();
            }
        } catch { /* silent */ }
    },
    async _loadSavedFromServer() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                this._updateSavedCount();
                if (this._activeTab === 'saved') this.renderSavedList();
            }
        } catch { /* silent */ }
    },
    _updateSharedCount() {
        const el = document.getElementById('col-shared-count');
        if (el) el.textContent = String(this.sharedCollections.length);
    },
    renderSharedList() {
        ...
        list.querySelectorAll('.col-open-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (slug) window.location.href = `/c/${slug}`;
            });
        });
        list.querySelectorAll('.collection-card').forEach(card => {
            card.addEventListener('click', (e) => {
    ...
    async _syncPublishedItems(col, colId, token) {
        if (!col || !col._publishedId || !token) return;
        try {
            ...
            btn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (res.ok) {
                        ...
                        if (sc) { sc.items = sc.items || []; sc.items.push(item); }
                        UI.showToast(`${this._t('collectionsAdded')} "${sc?.name || ''}"`);
                    } else {
                        const err = await res.json().catch(() => ({}));
                        UI.showToast(err.detail || 'Error');
                    }
                } catch { UI.showToast('Error'); }
                this._renderPickerList();
            });
        });
    },
