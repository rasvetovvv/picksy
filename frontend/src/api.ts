const API = {
    ...
    // ─── Movies ───
    async fetchMovies(filters: PickFilters = {}) {
    ...
    async pickMovie(filters: PickFilters = {}) {
        ...
        if (!this.hasApi.tmdb) {
            ...
            if (unseen.length === 0) this.seenMovieIds.clear();
        ...
        if (this.movieCache.length === 0) {
            if (this.moviePage === 1) this.moviePage = Math.floor(Math.random() * 15) + 1;
            this.movieCache = await this.fetchMovies(filters);
            if (this.movieCache.length === 0) {
                this.moviePage = 1;
                this.movieCache = await this.fetchMovies(filters);
            }
            if (this.movieCache.length === 0) return null;
        }
        ...
        if (unseen.length === 0) {
            ...
            this.movieCache = await this.fetchMovies(filters);
            if (this.movieCache.length === 0) this.movieCache = oldCache;
            ...
            if (pool.length === 0) return this.movieCache.length > 0
    ...
    // ─── TV Series ───
    async fetchTv(filters: PickFilters = {}) {
        const params = new URLSearchParams({
    ...
    async pickTv(filters: PickFilters = {}) {
        await this.checkBackend();
        ...
        if (!this.hasApi.tmdb) {
            ...
            if (unseen.length === 0) this.seenTvIds.clear();
            const pick = pool[Math.floor(Math.random() * pool.length)];
        ...
        if (this.tvCache.length === 0) {
            if (this.tvPage === 1) this.tvPage = Math.floor(Math.random() * 12) + 1;
            this.tvCache = await this.fetchTv(filters);
            if (this.tvCache.length === 0) {
                this.tvPage = 1;
                this.tvCache = await this.fetchTv(filters);
            }
            if (this.tvCache.length === 0) return null;
        }
    ...
    // ─── Books ───
    async fetchBooks(filters: PickFilters = {}) {
    ...
    async pickBook(filters: PickFilters = {}) {
        await this.checkBackend();
        ...
        if (!this.hasApi.books) {
            ...
            if (unseen.length === 0) this.seenBookIds.clear();
            const pick = pool[Math.floor(Math.random() * pool.length)];
        ...
        if (this.bookCache.length === 0) {
            if (this.bookPage === 1) this.bookPage = Math.floor(Math.random() * 6) + 1;
            this.bookCache = await this.fetchBooks(filters);
            if (this.bookCache.length === 0) {
                this.bookPage = 1;
                this.bookCache = await this.fetchBooks(filters);
            }
            if (this.bookCache.length === 0) return null;
        }
        ...
        if (unseen.length === 0) {
            ...
            this.bookCache = await this.fetchBooks(filters);
            if (this.bookCache.length === 0) this.bookCache = oldCache;
            ...
            if (pool.length === 0) return this.bookCache.length > 0
    ...
    async getHistory({ type = '', limit = 30, offset = 0 } = {}) {
        const auth = this._authHeaders();
        if (!auth) return { items: [], total: 0, limit, offset };
