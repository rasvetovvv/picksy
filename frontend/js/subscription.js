...
const Subscription = {
    ...
    async refresh() {
        if (!this.freemiumEnabled || !Auth.token)
            return;
        try {
    ...
    formatExpiryHTML() {
        ...
        if (days === 0)
            return `<span style="color:#fbbf24;font-weight:700">⏰ Закінчується сьогодні</span>`;
        if (days <= 3)
            return `<span style="color:#f87171;font-weight:700">⏰ Залишилось ${days} ${days === 1 ? 'день' : days < 5 ? 'дні' : 'днів'}</span> · до ${dateStr}`;
    ...
    _lockTierEl(node, need, isEn) {
        ...
        try {
            ...
            fields.forEach((f) => {
                const el = f;
                if (el.classList.contains('tier-locked-banner'))
                    return;
                if (el.dataset && !el.dataset.tierLockedFromDisabled) {
                    el.dataset.tierLockedFromDisabled = el.disabled ? '1' : '0';
                }
                el.disabled = true;
            });
        }
        ...
        let banner = node.querySelector(':scope > .tier-locked-banner');
        if (!banner) {
    ...
    _unlockTierEl(node) {
        ...
        try {
            ...
            fields.forEach((f) => {
                ...
                if (el.classList.contains('tier-locked-banner'))
                    return;
                const wasDisabled = el.dataset && el.dataset.tierLockedFromDisabled === '1';
                if (el.dataset && el.dataset.tierLockedFromDisabled !== undefined) {
                    delete el.dataset.tierLockedFromDisabled;
                }
        ...
        if (banner && banner.parentNode === node)
    ...
    renderBadge() {
        ...
        if (existing)
            existing.remove();
        if (!this.freemiumEnabled || !Auth.user)
            return;
    ...
    _t(uk, en) {
        try {
            if (typeof I18N !== 'undefined' && I18N && I18N.current === 'en')
                return en;
        }
        catch (e) { }
        return uk;
    ...
    selectDays(btn, tier, days) {
        ...
        if (planBtn)
            planBtn.dataset.selectedDays = days;
