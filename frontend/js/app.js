...
const App = {
    ...
    initHomeExperience() {
        if (this._homeExperienceInitialized)
            return;
        const main = document.querySelector('.main-content');
        const liveBar = document.getElementById('live-bar');
        if (!main || !liveBar)
            return;
        this._homeExperienceInitialized = true;
    ...
    initBrandQuoteRotation() {
        ...
        if (!el || el.dataset.rotateInit === '1')
            return;
        el.dataset.rotateInit = '1';
        const getQuotes = () => {
            try {
                const lang = (I18N && I18N.current) || 'uk';
                const arr = I18N?.translations?.[lang]?.brandQuotes;
    ...
    applyPageMode() {
        ...
        if (pageDesc)
            pageDesc.textContent = I18N.t(meta.desc);
        ...
        if (mode === 'watchlist') {
            ...
            document.querySelectorAll('.saved-cat-tab').forEach(tab => {
                tab.classList.toggle('active', tab.dataset.cat === 'watchlist');
            });
        ...
        if (mode === 'collections') {
            ...
            if (collBody)
                collBody.classList.remove('hidden');
            const collSection = document.getElementById('collections-section');
            if (collSection)
                collSection.style.removeProperty('display');
            try {
                if (typeof Collections !== 'undefined') {
                    if (!Collections._initialized)
                        Collections.init();
                    if (typeof Collections.render === 'function')
                        Collections.render();
                }
            }
            catch (e) { /* ignore */ }
        ...
        if (mode === 'notes') {
            setTimeout(() => document.getElementById('tracker-note')?.focus(), 250);
            this.renderNotes();
        }
        if (mode !== 'home') {
    ...
    navigateWorkspace(mode) {
        const path = this.WORKSPACE_ROUTES[mode] || '/';
        if (this.getPageMode() === mode)
            return false;
    ...
    initWorkspaceRouting() {
        if (this._workspaceRoutingBound)
            return;
        this._workspaceRoutingBound = true;
    ...
    updateDesktopWorkspaceNav(mode) {
        const normalized = mode === 'saved' ? 'watchlist' : mode;
        document.querySelectorAll('[data-workspace-link]').forEach((link) => {
            const key = link.dataset.workspaceLink || '';
            link.classList.toggle('active', key === normalized);
        });
    },
    initTheme() {
        ...
        document.querySelectorAll('.theme-btn[data-theme]').forEach((btn) => {
            btn.addEventListener('click', () => {
                this.applyTheme(btn.dataset.theme || 'black');
            });
        });
    },
    applyTheme(theme) {
    ...
    _initAiPoolIndices() {
        ...
        if (!this._aiChipIndices)
            this._aiChipIndices = shuffle(30);
        if (!this._aiMoodIndices)
            this._aiMoodIndices = shuffle(30);
    },
    _pickFromPool(pool, indices, count) {
        const out = [];
        const seen = new Set();
    ...
    updateAiMoods() {
        ...
        container.innerHTML = selected.map((m) => (`<button class="ai-mood-option" data-mood="${m.id}" data-q="${m.query.replace(/"/g, '"')}">${m.text}</button>`)).join('');
    },
    ...
    async syncFavorites() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (data.tv)
                    this.savedTv = data.tv;
                if (data.books)
                    this.savedBooks = data.books;
                this.updateSavedUI();
            }
        }
        catch {
    ...
    bindEvents() {
        ...
        document.getElementById('result-note-clear')?.addEventListener('click', () => {
            const input = document.getElementById('result-note-input');
            if (input)
                input.value = '';
        });
        ...
        document.getElementById('detail-btn')?.addEventListener('click', () => {
            if (this.currentItem)
                this.goToDetailPage(this.currentItem, this.currentType);
        });
        ...
        document.getElementById('result-card')?.addEventListener('click', (e) => {
            if (e.target.closest('button') || e.target.closest('a') || e.target.closest('.result-actions'))
                return;
            if (this.currentItem)
                this.goToDetailPage(this.currentItem, this.currentType);
        });
        ...
        UI.els.savedList.addEventListener('change', (e) => {
            ...
            if (!input)
                return;
            this.updateWatchedProgress(parseInt(input.dataset.index), input.dataset.field || '', parseInt(input.value || '0') || 0);
        });
        // Rating modal
        ...
        document.addEventListener('picksy:langchange', () => {
            try {
                ...
                if (this.currentMood !== null)
                    UI.setActiveMood(this.currentMood);
            }
            catch (e) { /* ignore */ }
            try {
                this.updateAiSuggestions();
            ...
            try {
                if (typeof Extras !== 'undefined') {
                    if (typeof Extras.renderDNA === 'function' && document.getElementById('dna-host'))
                        Extras.renderDNA();
                    if (typeof Extras.renderPsycho === 'function' && document.getElementById('psycho-host'))
                        Extras.renderPsycho();
                    if (typeof Extras.renderZodiac === 'function' && document.getElementById('zodiac-host'))
                        Extras.renderZodiac();
                }
            }
            catch (e) { /* ignore */ }
            ...
            try {
                if (typeof Auth !== 'undefined' && typeof Auth.renderProfileBadges === 'function') {
                    Auth.renderProfileBadges({ user: Auth.user });
                }
            }
            catch (e) { /* ignore */ }
        });
        ...
        if (aiMoodOptions) {
            aiMoodOptions.addEventListener('click', (e) => {
                const btn = e.target.closest('.ai-mood-option');
                if (!btn)
                    return;
    ...
    getMoodsForTab(tab) {
        if (tab === 'tv')
            return I18N.getTvMoods();
        if (tab === 'books')
            return I18N.getBookMoods();
    ...
    resetFilters() {
        ...
        if (UI.els.tvGenre)
            UI.els.tvGenre.value = '';
        if (UI.els.tvYearFrom)
            UI.els.tvYearFrom.value = '';
        if (UI.els.tvYearTo)
            UI.els.tvYearTo.value = '';
        if (UI.els.tvRating) {
            UI.els.tvRating.value = '0';
            UI.els.tvRatingVal.textContent = '0';
        }
        if (UI.els.tvCountry)
            UI.els.tvCountry.value = '';
        if (UI.els.bookSubject)
            UI.els.bookSubject.value = '';
        if (UI.els.bookQuery)
            UI.els.bookQuery.value = '';
        if (UI.els.bookLanguage)
            UI.els.bookLanguage.value = '';
        this.currentMood = null;
        UI.clearMood();
        API.reset();
    ...
    getShareUrl(item, type) {
        ...
        if (type === 'tv' && item.id)
            return `${base}/tv/${item.id}`;
        if (type === 'book' && item.id)
            return `${base}/book/${encodeURIComponent(item.id)}`;
    ...
    async _enrichPickWithDetails(item, type) {
        if (!item || !item.id)
            return;
        try {
    ...
    async pick() {
        ...
        try {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                ...
                if (!item)
                    break;
                if (!this.hideSeen || !this.isItemSaved(item, this.currentType))
                    break;
                item = null;
            }
            if (!item) {
            ...
            if (typeof Analytics !== 'undefined')
                Analytics.trackPick(this.currentType, item.title || '');
    ...
    async aiSearch() {
        ...
        const moodInput = document.getElementById('mood-ai-input');
    ...
    async toggleSave() {
        ...
        if (idx >= 0) {
            ...
            if (type === 'movie')
                await this.removeMovieFromServer(removed.id);
            else if (type === 'tv')
                await this.removeTvFromServer(removed.id);
            else
                await this.removeBookFromServer(removed.id);
        }
        else {
            ...
            if (type === 'movie')
                await this.saveMovieToServer(saved);
            else if (type === 'tv')
                await this.saveTvToServer(saved);
            else
                await this.saveBookToServer(saved);
            if (typeof Subscription !== 'undefined') {
                Subscription.savedTotal++;
                Subscription.renderBadge();
            }
            if (typeof Analytics !== 'undefined')
                Analytics.trackSave(type, item.title || '');
        }
        this.updateSavedUI();
    },
    openLink() {
    ...
    markAsWatched() {
        ...
        if (idx >= 0) {
            ...
            UI.updateSaveBtn(false);
            if (type === 'movie')
                this.removeMovieFromServer(item.id);
            else if (type === 'tv')
                this.removeTvFromServer(item.id);
            else
                this.removeBookFromServer(item.id);
        }
        this.updateSavedUI();
    },
    getWatchedList(type) {
    ...
    sortWatchedNewestFirst() {
        const newestFirst = (a, b) => new Date(b?.watchedAt || 0).getTime() - new Date(a?.watchedAt || 0).getTime();
    ...
    showRatingModal(index) {
        ...
        if (!item)
            return;
        if (item.userRating)
            this._ratingValue = item.userRating;
        const title = document.querySelector('.rating-modal-title');
        if (title)
            title.textContent = item.title || I18N.t('rateTitle');
    ...
    _trackerProgressLabel(item) {
        ...
        if (item.completed)
            return I18N.current === 'en' ? 'completed' : 'повністю';
        if (type === 'tv') {
    ...
    updateWatchedStats() {
        ...
        if (mc)
            mc.textContent = String(stats.month);
        if (yc)
            yc.textContent = String(stats.year);
        if (tc)
            tc.textContent = String(stats.total);
        this.updateTrackerUI();
    },
    initTracker() {
        const goalInput = document.getElementById('tracker-goal-input');
        const noteInput = document.getElementById('tracker-note');
    ...
    initNotes() {
        ...
        saveBtn?.addEventListener('click', () => {
            const input = document.getElementById('tracker-note');
        ...
        list?.addEventListener('click', (e) => {
            ...
            const saveEditBtn = target.closest('.note-edit-save');
            if (saveEditBtn) {
                this.saveEditedNote(saveEditBtn.dataset.id || '');
                return;
            }
            const cancelEditBtn = target.closest('.note-edit-cancel');
            if (cancelEditBtn) {
                this.cancelNoteEdit();
                return;
            }
            const deleteBtn = target.closest('.note-delete');
            if (deleteBtn) {
                this.deleteNote(deleteBtn.dataset.id || '');
                return;
            }
            const editBtn = target.closest('.note-edit');
            if (editBtn) {
                this.editNote(editBtn.dataset.id || '');
                return;
            }
            const detailBtn = target.closest('.note-detail');
            ...
            const noteOpen = target.closest('.note-poster, .note-top h3');
            if (noteOpen) {
    ...
    editNote(id) {
        ...
        this.renderNotes();
        setTimeout(() => {
            const input = Array.from(document.querySelectorAll('.note-edit-input')).find((el) => el.dataset.id === id);
            input?.focus();
    ...
    syncResultNotePanel() {
        ...
        if (meta)
            meta.textContent = `${title} · ${typeLabel}`;
        if (input)
            input.value = '';
    },
    focusResultNote() {
    ...
    addNoteFromCurrentResult() {
        ...
        const text = (input?.value || '').trim();
        if (!this.currentItem)
            return UI.showToast(I18N.t('saveHint'));
        if (!text) {
            UI.showToast(I18N.current === 'en' ? 'Write a note first' : 'Спочатку напиши нотатку');
            input?.focus();
            return;
    ...
    async hydrateNotesDetails() {
        if (this._notesHydrating || !Array.isArray(this.notes) || !this.notes.length)
            return;
        ...
        for (const note of needs.slice(0, 8)) {
            try {
                let data = null;
                if (note.itemType === 'tv')
                    data = await API.getTvDetails(note.itemId);
                else if (note.itemType === 'book')
                    data = await API.getBookDetails(note.itemId);
                else
                    data = await API.getMovieDetails(note.itemId);
                if (!data)
                    continue;
    ...
    renderNotes() {
        ...
        if (count)
            count.textContent = String((this.notes || []).length);
        if (!list)
            return;
    ...
    initMobileNav() {
        ...
        nav.querySelectorAll('.mobile-nav-item').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                const action = btn.dataset.nav || 'pick';
    ...
    updateMobileNavForMode(mode = this.getPageMode()) {
        ...
        const buttons = Array.from(nav.querySelectorAll('.mobile-nav-item'));
    ...
    _setMobileNavActive(action) {
        document.querySelectorAll('.mobile-nav-item').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.nav === action);
        });
    },
    handleMobileNav(action) {
        ...
        if (action === 'live')
            return this.openRecentPicksModal('all');
        if (action === 'new-note') {
            this.navigateWorkspace('notes');
            setTimeout(() => document.getElementById('tracker-note')?.focus(), 150);
            return;
        }
        if (action === 'watched')
            return this.openWatchedListFromTracker();
        if (action === 'quiz-archetype') {
            window.location.href = '/quiz';
            return;
        }
        if (action === 'quiz-match') {
        ...
        if (navigated) {
            if (mode === 'notes') {
                setTimeout(() => document.getElementById('tracker-note')?.focus(), 300);
            }
            return;
        }
        if (action === 'tracker' || action === 'notes') {
            ...
            if (action === 'notes') {
                setTimeout(() => document.getElementById('tracker-note')?.focus(), 350);
            }
            return;
        }
        if (action === 'saved') {
    ...
    openWatchedListFromTracker() {
        ...
        document.querySelectorAll('.saved-cat-tab').forEach(tab => {
            tab.classList.toggle('active', tab.dataset.cat === 'watched');
        });
        const statsEl = document.getElementById('watched-stats');
        if (statsEl)
    ...
    updateTrackerUI() {
        ...
        if (monthEl)
            monthEl.textContent = String(stats.month);
        if (yearEl)
            yearEl.textContent = String(stats.year);
        if (totalEl)
            totalEl.textContent = String(stats.total);
        if (fillEl)
            fillEl.style.width = `${progress}%`;
        if (recentCountEl)
            recentCountEl.textContent = String(stats.total);
        if (!listEl)
            return;
        ...
        const activeItems = allWatched.filter(item => !item.completed && (item.progressMinute || item.progressSeason || item.progressEpisode || item.progressPage));
        ...
        let insightsEl = root.querySelector('.tracker-insights');
    ...
    _normalizeDetailType(type) {
        ...
        if (raw === 'movies' || raw === 'movie')
            return 'movie';
        if (raw === 'tv' || raw === 'series' || raw === 'serial' || raw === 'shows')
            return 'tv';
    ...
    _getDetailUrl(itemOrId, type = '') {
        if (itemOrId && typeof itemOrId === 'object' && itemOrId.url) {
            try {
                ...
                if (/^\/desc(movie|tv|book)\//.test(parsed.pathname))
    ...
    _renderTmdbDetailPage(container, data, type) {
        ...
        if (data.tagline)
            html += `<div class="detail-tagline">${_escHtml(data.tagline)}</div>`;
        ...
        if (runtime)
            html += `<span class="detail-badge detail-badge-runtime">⏱️ ${_escHtml(runtime)}</span>`;
        if (seasons)
            html += `<span class="detail-badge detail-badge-runtime">📺 ${_escHtml(seasons)}</span>`;
        html += `</div>`;
        html += `</div></div>`;
        // Genres
        ...
        if (budget)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Бюджет' : 'Budget'}</div><div class="detail-meta-value">${_escHtml(budget)}</div></div>`;
        if (revenue)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Касові збори' : 'Revenue'}</div><div class="detail-meta-value">${_escHtml(revenue)}</div></div>`;
        if (episodes)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Епізоди' : 'Episodes'}</div><div class="detail-meta-value">${_escHtml(episodes)}</div></div>`;
        if (data.status)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Статус' : 'Status'}</div><div class="detail-meta-value">${_escHtml(data.status)}</div></div>`;
        ...
        if (type === 'tv' && data.status) {
            ...
            if (data.number_of_seasons)
                html += `<span class="detail-tv-stat">${data.number_of_seasons} ${isUk ? 'сезонів' : 'seasons'}</span>`;
            if (data.number_of_episodes)
                html += `<span class="detail-tv-stat">${data.number_of_episodes} ${isUk ? 'епізодів' : 'episodes'}</span>`;
            html += `</div>`;
        ...
        if (data.tmdb_reviews && data.tmdb_reviews.length) {
            ...
            data.tmdb_reviews.forEach(r => {
                ...
                if (r.rating)
                    html += `<span class="detail-review-rating">⭐ ${r.rating}/10</span>`;
    ...
    _renderBookDetailPage(container, data) {
        ...
        html += `<h1 class="detail-title">${_escHtml(title)}</h1>`;
        if (data.subtitle)
            html += `<div class="detail-tagline">${_escHtml(data.subtitle)}</div>`;
        html += `<div class="detail-badges">`;
        if (rating !== '—')
            html += `<span class="detail-badge detail-badge-rating">⭐ ${_escHtml(rating)}</span>`;
        html += `<span class="detail-badge detail-badge-year">📅 ${_escHtml(year)}</span>`;
        if (data.page_count)
            html += `<span class="detail-badge detail-badge-runtime">📄 ${data.page_count} ${I18N.current === 'uk' ? 'стор.' : 'pages'}</span>`;
        html += `</div>`;
        html += `</div></div>`;
        // Categories
        ...
        if (data.authors && data.authors.length)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Автори' : 'Authors'}</div><div class="detail-meta-value">${_escHtml(data.authors.join(', '))}</div></div>`;
        if (data.publisher)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Видавництво' : 'Publisher'}</div><div class="detail-meta-value">${_escHtml(data.publisher)}</div></div>`;
        if (data.isbn)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">ISBN</div><div class="detail-meta-value">${_escHtml(data.isbn)}</div></div>`;
        if (data.language)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Мова' : 'Language'}</div><div class="detail-meta-value">${_escHtml(data.language.toUpperCase())}</div></div>`;
        if (data.ratings_count)
            html += `<div class="detail-meta-item"><div class="detail-meta-label">${I18N.current === 'uk' ? 'Кількість оцінок' : 'Ratings'}</div><div class="detail-meta-value">${data.ratings_count}</div></div>`;
        html += '</div>';
        // Community Rating
        html += `<div class="detail-community-section">`;
        html += `<div class="detail-section-title">💜 ${I18N.current === 'uk' ? 'Рейтинг Picksy' : 'Picksy Rating'}</div>`;
    ...
    async submitDetailRating(contentType, contentId, rating) {
        ...
        if (result) {
            ...
            if (scoreEl && updated) {
                ...
                if (countEl)
                    countEl.textContent = `${updated.count} ${I18N.current === 'uk' ? 'оцінок' : 'votes'}`;
            }
        }
    },
    async submitDetailReview(contentType, contentId) {
        ...
        const textarea = document.getElementById('detail-review-text');
    ...
    async analyzeMoodText() {
        const input = document.getElementById('mood-ai-input');
        ...
        if (aiType === 'book' && result.subject) {
            if (UI.els.bookSubject)
                UI.els.bookSubject.value = result.subject;
        }
    ...
    renderMovieNight() {
        ...
        if (mn.phase === 'setup') {
            ...
            document.getElementById('mn-add-participant').addEventListener('click', () => {
                const input = document.getElementById('mn-name-input');
            ...
            document.getElementById('mn-name-input').addEventListener('keydown', (e) => {
                if (e.key === 'Enter')
                    document.getElementById('mn-add-participant').click();
            });
            ...
            mn.participants.forEach(p => {
                (mn.selections[p] || []).forEach(m => {
                    if (!allMovies.some(x => x.id === m.id))
                        allMovies.push({ ...m, nominatedBy: p });
                });
            });
            const voteCounts = {};
            allMovies.forEach(m => { voteCounts[m.id] = 0; });
            Object.values(mn.votes).forEach(movieId => { if (voteCounts[movieId] !== undefined)
                voteCounts[movieId]++; });
            ...
            container.querySelectorAll('.mn-vote-card').forEach(card => {
                card.addEventListener('click', () => {
                    const voter = document.getElementById('mn-voter-select').value;
                    if (mn.votes[voter] !== undefined)
                        return;
            ...
            if (resultBtn)
                resultBtn.addEventListener('click', () => {
                    ...
                    allMovies.forEach(m => { if (voteCounts2[m.id] > maxVotes)
                        maxVotes = voteCounts2[m.id]; });
            ...
            mn.countdownInterval = setInterval(() => {
                ...
                if (rem <= 0) {
                    clearInterval(mn.countdownInterval);
                    if (timeEl)
                        timeEl.textContent = '🎬 ' + I18N.t('timeToWatch') + '!';
                }
            }, 1000);
    ...
    renderDuo() {
        ...
        if (d.phase === 'lobby') {
            ...
            const codeInput = document.getElementById('duo-code-input');
        ...
        if (d.phase === 'waiting') {
            ...
            document.getElementById('duo-code-copy').addEventListener('click', () => {
                navigator.clipboard.writeText(d.code).then(() => {
                    ...
                    if (el) {
                        el.textContent = '✓ ' + t('duoCopied');
                        setTimeout(() => { el.textContent = d.code; }, 1500);
                    }
                });
            });
            this._duoPoll();
        ...
        if (d.phase === 'prefs') {
            ...
            document.getElementById('duo-genre-grid').addEventListener('click', (e) => {
                const chip = e.target.closest('.duo-genre-chip');
                if (!chip)
                    return;
        ...
        if (d.phase === 'result' && d.result) {
            ...
            const genreChips = genres.map((g) => `<span class="duo-meta-chip genre">${g}</span>`).join('');
            ...
            if (newRoomBtn)
                newRoomBtn.addEventListener('click', () => { this.openDuo(); });
            ...
            if (card && detailHref) {
                ...
                card.addEventListener('click', (ev) => {
                    ...
                    if (tgt.closest('.duo-result-actions'))
                        return;
                    // Don't hijack text selection inside the overview.
                    const sel = window.getSelection && window.getSelection();
                    if (sel && sel.toString() && tgt.closest('.duo-movie-overview-wrap'))
                        return;
                    go();
                });
    ...
    _duoHandleAuthError() {
        if (this._duo && this._duo.pollTimer)
            clearInterval(this._duo.pollTimer);
        try {
            UI.showToast(I18N.t('duoLoginRequired'));
        }
    ...
    async _duoSubmitPrefs() {
        const moodInput = document.getElementById('duo-mood-input');
    ...
    normalizeMediaType(type) {
        ...
        if (value === 'tv' || value === 'series' || value === 'serial' || value === 'shows')
            return 'tv';
        if (value === 'book' || value === 'books' || value === 'volume')
            return 'book';
        return 'movie';
    },
    activateSavedCategory(category) {
        ...
        const statsEl = document.getElementById('watched-stats');
        if (statsEl)
            statsEl.classList.toggle('hidden', this.savedCategory !== 'watched');
    },
    removeSaved(index) {
        if (this.savedCategory === 'watched') {
        ...
        if (removed) {
            const t = this.typeForSavedTab(this.savedTab);
            if (t === 'movie')
                this.removeMovieFromServer(removed.id);
            else if (t === 'tv')
                this.removeTvFromServer(removed.id);
            else
                this.removeBookFromServer(removed.id);
        }
        this.updateSavedUI();
        this.renderSaved();
        if (this.currentItem)
            UI.updateSaveBtn(this.isItemSaved(this.currentItem, this.currentType));
    },
    ...
    async loadDailyPick(tabType) {
        ...
        try {
            ...
            const posterEl = document.getElementById('daily-pick-poster');
            posterEl.src = poster;
    ...
    _dailyPickTimerInterval: null,
    _startDailyPickTimer() {
        if (this._dailyPickTimerInterval)
            clearInterval(this._dailyPickTimerInterval);
    ...
    initFab() {
        ...
        document.getElementById('fab-admin')?.addEventListener('click', () => {
            ...
            const isSuper = role === 'superadmin' || Auth?.user?.is_admin;
        ...
        saveBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    if (hint)
                        hint.textContent = data.detail || 'Error';
                    return;
                }
                const hint = document.getElementById('profile-username-hint');
                if (hint)
    ...
    initProfileEditor() {
        ...
        if (favRemoveBtn) {
            favRemoveBtn.addEventListener('click', () => {
                ...
                const commentEl = document.getElementById('profile-fav-film-comment');
                if (commentEl)
                    commentEl.value = '';
        ...
        if (save) {
            save.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (avatar && payload.avatar_emoji)
                        avatar.textContent = payload.avatar_emoji;
                    ...
                    try {
                        const recs = (typeof window.App !== 'undefined' && Array.isArray(window.App._profileRecs))
                            ? window.App._profileRecs : [];
        ...
        if (copy) {
            copy.addEventListener('click', async () => {
                ...
                if (!link || !link.href)
                    return UI.showToast('Спочатку збережи username');
                try {
    ...
    applySiteSettings(settings) {
        ...
        if (aiBox && settings.ai_enabled === false)
        ...
        if (achievementsSection)
            achievementsSection.classList.toggle('hidden', settings.achievements_enabled === false);
        // Feedback visibility
        ...
        if (feedbackSection)
            feedbackSection.classList.toggle('hidden', !feedbackEnabled);
        const feedbackMiniBtn = document.getElementById('feedback-mini-btn');
        if (feedbackMiniBtn)
            feedbackMiniBtn.classList.toggle('hidden', !feedbackEnabled);
        const feedbackFab = document.getElementById('feedback-fab');
        if (feedbackFab)
            feedbackFab.classList.toggle('hidden', !feedbackEnabled);
        ...
        if (gameBtn)
            gameBtn.classList.toggle('hidden', !gameEnabled);
        if (fabGame)
            fabGame.classList.toggle('hidden', !gameEnabled);
        if (gameProfileSection)
            gameProfileSection.classList.toggle('hidden', !gameEnabled);
    ...
    renderPopupAd(settings) {
        ...
        for (let i = localStorage.length - 1; i >= 0; i--) {
            ...
            if (k && k.startsWith('picksy_popup_seen_') && k !== seenKey)
                localStorage.removeItem(k);
        }
        if (!settings.popup_force && localStorage.getItem(seenKey) === '1')
            return;
        if (!wrap) {
        ...
        if (close)
            close.addEventListener('click', () => { localStorage.setItem(seenKey, '1'); wrap.remove(); });
    },
    ...
    async loadGameRatingForProfile() {
        try {
            ...
            if (recordEl)
                recordEl.textContent = `${d.wins}W / ${d.losses}L / ${d.draws}D`;
        }
        catch { }
    },
    // ─── Scroll to Top ───
    ...
    async loadStreak() {
        ...
        try {
            ...
            if (streakContainer) {
                ...
                if (cls)
                    streakContainer.classList.add(cls);
            }
            if (fireEl) {
                ...
                if (!item.description && item.id) {
                    try {
                        ...
                        if (detailRes.ok) {
                            ...
                            if (detail.genre_ids)
                                item.genre_ids = detail.genre_ids;
    ...
    async loadAchievements() {
        ...
        try {
            ...
            if (grid) {
                const lang = (typeof I18N !== 'undefined') ? I18N.current : 'uk';
                grid.innerHTML = list.map(a => `<div class="achievement-item ${a.earned ? 'earned' : 'locked'}" title="${lang === 'en' ? a.desc_en : a.desc_uk}">` +
    ...
    _renderShowcaseGrid() {
            ...
            btn.addEventListener('click', (ev) => {
                ...
                if (el.classList.contains('not-earned')) {
                    if (typeof UI !== 'undefined' && UI.showToast)
                        UI.showToast(lockedNotEarnedTitle);
                    return;
                }
        ...
        const hintEl = document.querySelector('.profile-showcase-hint');
        if (hintEl) {
    ...
    async _saveShowcaseBadges(badge_ids) {
        ...
        try {
            ...
            if (!res.ok) {
                if (typeof UI !== 'undefined' && UI.showToast)
                    UI.showToast(I18N.t('saveError') || 'Помилка збереження');
                return;
            }
            const data = await res.json();
    ...
    initGuestPicksBadge() {
        ...
        if (pickBtns)
            pickBtns.appendChild(badge);
    },
    _updateGuestBadge(badge) {
        if (!badge)
            badge = document.getElementById('guest-picks-badge');
        if (!badge || Auth.user) {
            if (badge)
                badge.remove();
            return;
    ...
    initDonate() {
        ...
        if (supportBtn)
            supportBtn.addEventListener('click', () => modal.classList.remove('hidden'));
        if (overlay)
            ...
            switch (e.key.toLowerCase()) {
                ...
                case 's':
                    ...
                    if (this.currentItem) {
                        ...
                        if (!saved)
                            this.saveItem(this.currentItem, this.currentType);
                        else
                            this.removeItem(this.currentItem, this.currentType);
                    }
                    break;
                case 'f':
                    e.preventDefault();
                ...
                case '1':
                    ...
                    document.querySelectorAll('.tab-btn').forEach(b => {
                        b.classList.toggle('active', b.dataset.tab === 'movies');
                    });
                    break;
                case '2':
                    e.preventDefault();
                    this.switchTab('tv');
                    document.querySelectorAll('.tab-btn').forEach(b => {
                        b.classList.toggle('active', b.dataset.tab === 'tv');
                    });
                    break;
                case '3':
                    e.preventDefault();
                    ...
                    document.querySelectorAll('.tab-btn').forEach(b => {
                        b.classList.toggle('active', b.dataset.tab === 'books');
                    });
                    break;
                case '?':
                    showKbdHint();
    ...
    initCookieConsent() {
        if (localStorage.getItem('picksy_cookies_accepted'))
            return;
    ...
    _liveTimer: null,
    _livePopularTimer: null,
    _liveSSE: null,
    _liveLastPickAt: null,
    _liveRecentTickTimer: null,
    _liveTeardownBound: false,
    _liveRecentItems: null,
    async _fetchLiveSnapshot() {
        ...
        const out = { online: 0, last_pick_at: null };
        try {
    ...
    _tickLastPick(recentEl) {
        if (!recentEl)
            recentEl = document.getElementById('live-recent');
        if (!recentEl || !this._liveLastPickAt)
            return;
        const t = typeof I18N !== 'undefined' ? (k) => I18N.t(k) : (k) => k;
    ...
    renderRecentPicks(type = 'all') {
        ...
        document.querySelectorAll('.recent-picks-filter').forEach((btn) => {
            btn.classList.toggle('active', btn.dataset.rptype === type);
        });
        const items = (this._liveRecentItems || [])
    ...
    initLiveBar() {
        ...
        if (!this._recentPicksBound) {
            ...
            document.querySelectorAll('.recent-picks-filter').forEach((btn) => {
                btn.addEventListener('click', () => this.renderRecentPicks(btn.dataset.rptype || 'all'));
            });
            document.getElementById('recent-picks-list')?.addEventListener('click', (e) => {
                const card = e.target.closest('.recent-pick-item');
                if (!card)
                    return;
